...
to_plot = sc.objdict({
    ...
    'Daily hospitalisations': ['new_critical'],
...
# Filepaths
data_path = 'England_Covid_cases_june11.xlsx'
resfolder = 'results_delay'

# Important dates
start_day = '2020-01-21'
end_day = '2021-10-30'
data_end = '2021-06-11' # Final date for calibration
   ...
   # ADD BETA INTERVENTIONS
    ...
    if not calibration:
        ...
        if scenario == 'Roadmap_All':
            ...
            beta_scens = sc.odict({'2021-05-03': [1.25, sbv, 0.40, 0.60],
                               '2021-05-10': [1.25, sbv, 0.40, 0.60],
                               '2021-05-17': [1.25, sbv, 0.50, 0.70],
                               '2021-05-21': [1.25, sbv, 0.50, 0.70],
                               '2021-05-28': [1.25, 0.02, 0.50, 0.70],
        ...
        ## Projecting until end of 2021.
            ...
            beta_scens = sc.odict({'2021-04-12': [1.25, 0.02, 0.40, 0.60],
                               '2021-04-19': [1.25, sbv, 0.40, 0.60],
                               '2021-04-26': [1.25, sbv, 0.40, 0.60],
                               '2021-05-03': [1.25, sbv, 0.40, 0.60],
                               '2021-05-10': [1.25, sbv, 0.40, 0.60],
    ...
    # Add B.1.117 strain
    b117 = cv.strain('b117', days=np.arange(sim.day('2020-08-01'), sim.day('2020-08-10')), n_imports=500)
    b117.p['rel_beta']        = 1.4
    b117.p['rel_death_prob']  = 1.0
    b117.p['rel_severe_prob'] = 1.0
    sim['strains'] += [b117]
    # Add B.1.1351 strain
    b1351 = cv.strain('b1351', days=np.arange(sim.day('2020-01-10'), sim.day('2020-01-20')), n_imports=1000)
    b1351.p['rel_beta']        = 1.0
    b1351.p['rel_death_prob']  = 1.0
    b1351.p['rel_severe_prob'] = 1.0
    sim['strains'] += [b1351]
    # Add B.X.XXX strain starting middle of March
    custom_strain = cv.strain(label='custom', strain = cvp.get_strain_pars()['p1'],
                              days=np.arange(sim.day('2021-03-17'), sim.day('2021-03-24')), n_imports=2000)
    custom_strain.p['rel_beta']        = 2.0
    custom_strain.p['rel_death_prob']  = 1.0
    ...
    sim.init_immunity()
    sim['immunity']
    prior = {'wild': 0.8, 'b117': 0.8, 'b1351': 0.8, 'custom': 0.8}
    pre = {'wild': 0.8, 'b117': 0.8, 'b1351': 0.8, 'custom':0.8}
    ...
    # hard code them in
    dose_pars = {'nab_eff': {'sus': {'slope': 1.6, 'n_50': 0.05}},
                 'nab_init': {'dist': 'normal', 'par1': -0.85, 'par2': 2},
                 'nab_boost': 3,
                 'doses': 2,
                 'interval': 7*12,
                 'nab_interval': 14}
    strain_pars = {'wild':1.0, 'b117':1.0, 'b1351':1/9, 'p1':0.8, 'custom':1.0}
    vaccine = sc.mergedicts({'label':'az_uk'}, sc.mergedicts(dose_pars, strain_pars))
    ...
    def subtarget_75_100(sim):
        ...
        if not hasattr(sim, 'subtarget_75_100'):
            sim.subtarget_75_100 = cv.binomial_filter(0.05, inds) # 30% of 70+ years olds will not agree to get vaccinated
    ...
    def subtarget_60_75(sim):
        ...
        if not hasattr(sim, 'subtarget_60_75'):
            sim.subtarget_60_75 = cv.binomial_filter(0.05, inds) # 5% of 70+ years olds will not agree to get vaccinated
        inds = np.setdiff1d(inds, sim.subtarget_60_75)
        return {'inds': inds, 'vals': 0.020*np.ones(len(inds))}
    interventions += [utils_vac.vaccinate(vaccine=vaccine, prob=0.1, subtarget=subtarget_60_75,
                                   days=np.arange(sim.day('2021-01-28'), sim.day('2023-01-01')))]
    
    def subtarget_50_60(sim):
        inds = cv.true((sim.people.age >= 50) & (sim.people.age < 60))
        if not hasattr(sim, 'subtarget_50_60'):
            sim.subtarget_50_60 = cv.binomial_filter(0.3, inds) # 20% of 50-60 years olds will not agree to get vaccinated
        inds = np.setdiff1d(inds, sim.subtarget_50_60)
        return {'inds': inds, 'vals': 0.020*np.ones(len(inds))}
    interventions += [utils_vac.vaccinate(vaccine=vaccine, prob=0.1, subtarget=subtarget_50_60,
                                   days=np.arange(sim.day('2021-02-10'), sim.day('2023-01-01')))]
    def subtarget_40_50(sim):
        inds = cv.true((sim.people.age >= 40) & (sim.people.age < 50))
        if not hasattr(sim, 'subtarget_40_50'):
            sim.subtarget_40_50 = cv.binomial_filter(0.50, inds) # 30% of 40-50 years olds will not agree to get vaccinated
        inds = np.setdiff1d(inds, sim.subtarget_40_50)
        return {'inds': inds, 'vals': 0.01*np.ones(len(inds))}
    interventions += [utils_vac.vaccinate(vaccine=vaccine, prob=0.1, subtarget=subtarget_40_50,
    ...
    def subtarget_30_40(sim):
        ...
        if not hasattr(sim, 'subtarget_30_40'):
            sim.subtarget_30_40 = cv.binomial_filter(0.7, inds) # 30% of 30-40+ years olds will not agree to get vaccinated
        inds = np.setdiff1d(inds, sim.subtarget_30_40)
        return {'inds': inds, 'vals': 0.01*np.ones(len(inds))}
    interventions += [utils_vac.vaccinate(vaccine=vaccine, prob=0.1, subtarget=subtarget_30_40,
                                   days=np.arange(sim.day('2021-05-10'), sim.day('2023-01-01')))]
    def subtarget_25_30(sim):
        inds = cv.true((sim.people.age >= 25) & (sim.people.age < 30))
        if not hasattr(sim, 'subtarget_25_30'):
            sim.subtarget_25_30 = cv.binomial_filter(0.9, inds) # 30% of 18+ years olds will not agree to get vaccinated
        inds = np.setdiff1d(inds, sim.subtarget_25_30)
        return {'inds': inds, 'vals': 0.005*np.ones(len(inds))}
    interventions += [utils_vac.vaccinate(vaccine=vaccine, prob=0.01, subtarget=subtarget_25_30,
                                   days=np.arange(sim.day('2021-06-10'), sim.day('2023-01-01')))]
    
    def subtarget_18_25(sim):
        inds = cv.true((sim.people.age >= 18) & (sim.people.age < 25))
        if not hasattr(sim, 'subtarget_18_30'):
            sim.subtarget_18_25 = cv.binomial_filter(0.9, inds) # 30% of 18+ years olds will not agree to get vaccinated
        inds = np.setdiff1d(inds, sim.subtarget_18_25)
        return {'inds': inds, 'vals': 0.005*np.ones(len(inds))}
    interventions += [utils_vac.vaccinate(vaccine=vaccine, prob=0.01, subtarget=subtarget_18_25,
                                   days=np.arange(sim.day('2021-06-20'), sim.day('2023-01-01')))]
...
if __name__ == '__main__':
    ...
    if whattorun=='quickfit':
        s0 = make_sim(seed=1, beta=0.0078, end_day='2021-09-20', verbose=0.1)
        sims = []
        ...
        if do_plot:
            ...
            msim.plot('strains', do_save=True, do_show=False, fig_path=f'uk_strain.png')
    # Run scenarios
        ...
        for scenname in scenarios:
            ...
            print('---------------\n')
            sc.blank()
            s0 = make_sim(seed=1, beta=0.0079, end_day='2022-01-10', calibration=False, scenario=scenname, verbose=0.1)
            s0.run(until='2022-01-25')
            sims = []
