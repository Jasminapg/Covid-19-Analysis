...
# Filepaths
data_path = 'England_Covid_cases_may02.xlsx'
resfolder = 'results_delay'

# Important dates
start_day = '2020-01-21'
end_day = '2021-10-30'
data_end = '2021-05-05' # Final date for calibration
...
def make_sim(seed, beta, calibration=True, future_symp_test=None, scenario=None, end_day='2021-10-30', verbose=0):
   ...
   # ADD BETA INTERVENTIONS
    ...
    # Add B.1.117 strain
    b117 = cv.strain('b117', days=np.arange(sim.day('2020-09-01'), sim.day('2020-09-10')), n_imports=100)
    sim['strains'] += [b117]
    # Add B.1.1351 strain
    b1351 = cv.strain('b1351', days=np.arange(sim.day('2020-11-20'), sim.day('2020-11-30')), n_imports=600)
    sim['strains'] += [b1351]
    # Add B.X.XXX strain starting middle of April
    custom_strain = cv.strain(label='custom', strain = cvp.get_strain_pars()['p1'],
                              days=np.arange(sim.day('2021-03-10'), sim.day('2021-03-10')), n_imports=600)
    ...
    interventions += [
        ...
        cv.test_prob(symp_prob=s_prob_jan, asymp_prob=0.012, symp_quar_prob=0.0, start_day=tti_day_feb, end_day=tti_day_march-1, test_delay=t_delay),
        cv.test_prob(symp_prob=s_prob_jan, asymp_prob=0.012, symp_quar_prob=0.0, start_day=tti_day_march, end_day=tti_day_april-1, test_delay=t_delay),
        cv.test_prob(symp_prob=s_prob_jan, asymp_prob=0.012, symp_quar_prob=0.0, start_day=tti_day_april, test_delay=t_delay),  
    ...
    dose_pars = {'nab_eff': {'sus': {'slope': 1.6, 'n_50': 0.05}},
                 ...
                 'interval': 7*12,
                 'nab_interval': 14}
    strain_pars = {'wild':1.0, 'b117':1/2.3, 'b1351':1/9, 'p1':1/2.9, 'custom':1/2.3}
    ...
    def subtarget_18_30(sim):
        ...
        return {'inds': inds, 'vals': 0.003*np.ones(len(inds))}
    interventions += [utils_vac.vaccinate(vaccine=vaccine, prob=0.01, subtarget=subtarget_18_30,
