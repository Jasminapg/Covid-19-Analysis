...
class two_dose_daily_delayed(cv.Intervention):
    ...
    def __init__(self, daily_vaccines, prob=1.0, rel_symp=0.0, rel_trans=1.0, take_prob=1.0, delay=7*7, dose_delay=14,
                 age_priority=65, priority_days=None, cumulative=[0.5, 1.0], dose_priority=[1.0, 5.0], start_day=0, end_day=None, **kwargs):
        super().__init__(**kwargs)  # Initialize the Intervention object
        self._store_args()  # Store the input arguments so the intervention can be recreated
        ...
        self.age_priority = sc.promotetolist(age_priority)
        self.priority_days = priority_days
    ...
    def apply(self, sim):
        ...
        # age priority
        ppl = sim.people
        priority_stage = sc.findlast(self.priority_days <= t)
        not_target_inds = cv.true(np.logical_or(ppl.age < self.age_priority[priority_stage], ppl.dead))
        vacc_probs[not_target_inds] = 0.0
