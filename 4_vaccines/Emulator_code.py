...
# Filepaths
data_path = 'England_Covid_cases_june2021.xlsx'
   ...
   # ADD BETA INTERVENTIONS
    ...
    if not calibration:
        ...
        if scenario == 'Roadmap_Step3':
            ...
            beta_scens = sc.odict({'2021-06-21': [1.25, sbv, 0.30, 0.50],
...
if __name__ == '__main__':
    ...
    if whattorun=='quickfit':
        ...
        if save_sim:
                    #msim.reduce()
                    msim.reduce(quantiles=[0.10, 0.90])
                    msim.save(f'{resfolder}/uk_sim_test.obj',keep_people=True)
        if do_plot:
            msim.plot(to_plot=to_plot, do_save=True, do_show=False, fig_path='England_test.png',
                      legend_args={'loc': 'upper left'}, axis_args={'hspace': 0.4}, interval=75, n_cols=2)
            msim.plot('variants', do_save=True, do_show=False, fig_path='uk_strain_test.png')
    # Run scenarios
