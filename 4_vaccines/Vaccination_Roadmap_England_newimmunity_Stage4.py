...
# Filepaths
data_path = 'England_Covid_cases_june19.xlsx'
...
def make_sim(seed, beta, calibration=True, future_symp_test=None, scenario=None, end_day='2021-10-30', verbose=0):
   ...
   # ADD BETA INTERVENTIONS
    ...
    beta_past  = sc.odict({'2020-02-14': [1.00, 1.00, 0.90, 0.90],
                           ...
                           '2021-04-12': [1.25, 0.02, 0.40, 0.50],
                           ...
                           '2021-05-28': [1.15, 0.02, 0.40, 0.60],
    ...
    b117.p['rel_beta']        = 1.5
    b117.p['rel_crit_prob']   = 1.5
    b117.p['rel_death_prob']  = 1.5
    b117.p['rel_severe_prob'] = 0.3
    sim['variants'] += [b117]
    # Add B.1.1351 strain
    b1351 = cv.variant('b1351', days=np.arange(sim.day('2020-01-10'), sim.day('2020-01-20')), n_imports=1000)
    b1351.p['rel_beta']        = 1.0
    b1351.p['rel_death_prob']  = 1.0
    b1351.p['rel_severe_prob'] = 1.0
    sim['variants'] += [b1351]
    # Add B.X.XXX strain starting middle of March
    custom_strain = cv.variant(label='custom', variant = cvp.get_variant_pars()['p1'],
    ...
    custom_strain.p['rel_severe_prob'] = 2.1
    ...
    def subtarget_60_75(sim):
        ...
        return {'inds': inds, 'vals': 0.015*np.ones(len(inds))}
    ...
    def subtarget_50_60(sim):
        ...
        if not hasattr(sim, 'subtarget_50_60'):
            sim.subtarget_50_60 = cv.binomial_filter(0.07, inds) # 20% of 50-60 years olds will not agree to get vaccinated
    ...
    def subtarget_45_50(sim):
        ...
        if not hasattr(sim, 'subtarget_45_50'):
            sim.subtarget_45_50 = cv.binomial_filter(0.07, inds) # 30% of 40-50 years olds will not agree to get vaccinated
    ...
    interventions += [cv.vaccinate(vaccine=az_vaccine, prob=0.1, subtarget=subtarget_45_50,
                                   days=np.arange(sim.day('2021-03-20'), sim.day('2023-01-01')))]
    
    def subtarget_40_45(sim):
        inds = cv.true((sim.people.age >= 40) & (sim.people.age < 45))
        if not hasattr(sim, 'subtarget_40_45'):
            sim.subtarget_40_45 = cv.binomial_filter(0.07, inds) # 30% of 40-50 years olds will not agree to get vaccinated
    ...
    def subtarget_30_40(sim):
        ...
        if not hasattr(sim, 'subtarget_30_40'):
            sim.subtarget_30_40 = cv.binomial_filter(0.1, inds) # 30% of 30-40+ years olds will not agree to get vaccinated
    ...
    def subtarget_25_30(sim):
        ...
        if not hasattr(sim, 'subtarget_25_30'):
            sim.subtarget_25_30 = cv.binomial_filter(0.1, inds) # 30% of 18+ years olds will not agree to get vaccinated
    ...
    def subtarget_18_25(sim):
        ...
        if not hasattr(sim, 'subtarget_18_30'):
            sim.subtarget_18_25 = cv.binomial_filter(0.1, inds) # 30% of 18+ years olds will not agree to get vaccinated
    ...
    #    return {'inds': inds, 'vals': 0.002*np.ones(len(inds))}
    #interventions += [cv.vaccinate(vaccine=pfizer_vaccine, prob=0.1, subtarget=subtarget_18_25,
    #                               days=np.arange(sim.day('2021-07-20'), sim.day('2023-01-01')))]
...
if __name__ == '__main__':
    ...
    if whattorun=='quickfit':
        s0 = make_sim(seed=1, beta=0.0078, end_day='2021-06-21', verbose=0.1)
        sims = []
        ...
        if do_plot:
            ...
            msim.plot('variants', do_save=True, do_show=False, fig_path=f'uk_strain.png')
    # Run scenarios
        ...
        for scenname in scenarios:
            ...
            print('---------------\n')
            sc.blank()
            s0 = make_sim(seed=10, beta=0.0078, end_day='2021-12-31', calibration=False, scenario=scenname, verbose=0.1)
