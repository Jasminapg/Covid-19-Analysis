...
verbose = 1
seed    = 1
keep_people = 1 # Whether to keep people
to_plot = sc.objdict({
...
# Filepaths
data_path = 'England_Covid_cases_oct18.xlsx'
resfolder = 'results_Vac'

# Important dates
start_day = '2020-01-21'
end_day = '2021-10-30'
data_end = '2021-10-04' # Final date for calibration
...
def make_sim(seed, beta, calibration=True, future_symp_test=None, scenario=None, vx_scenario=None, end_day='2021-10-30', verbose=0):
   ...
   # ADD BETA INTERVENTIONS
    ...
    beta_past  = sc.odict({'2020-02-14': [1.00, 1.00, 0.90, 0.90],
                           ...
                           '2021-06-07': [1.05, 0.50, 0.30, 0.50],
                           '2021-06-14': [1.05, 0.50, 0.30, 0.50],
                           ...
                           '2021-06-19': [1.05, 0.50, 0.30, 0.50],
                           '2021-06-21': [1.05, 0.50, 0.30, 0.50],
                           '2021-06-28': [1.05, 0.50, 0.30, 0.50],
                           '2021-07-05': [1.05, 0.50, 0.30, 0.50],
                           '2021-07-12': [1.05, 0.40, 0.30, 0.50],
                           #cases start to drop from middle of July
                           '2021-07-16': [1.05, 0.00, 0.30, 0.70],
                           #easing of socal distancing measures - delayed step 4
                           '2021-07-26': [1.05, 0.00, 0.40, 0.70],
                           '2021-08-02': [1.05, 0.00, 0.40, 0.70],
                           '2021-08-09': [1.05, 0.00, 0.40, 0.70],
                           '2021-08-16': [1.05, 0.00, 0.40, 0.70],
                           '2021-08-23': [1.05, 0.00, 0.40, 0.70],
                           ...
                           '2021-10-13': [1.05, sbv_new, 0.50, 0.80],
                           '2021-10-25': [1.05, 0.00, 0.50, 0.80],
    ...
    b16172 = cv.variant('b16172', days=np.arange(sim.day('2021-03-17'), sim.day('2021-03-20')), n_imports=700)
    ...
    tti_day_july21= sim.day('2021-07-12')
    ...
    s_prob_dec = 0.07769
    s_prob_jan = 0.07769
    s_prob_march = 0.045069
    s_prob_july21 = 0.045069
    s_prob_august21 = 0.02769
    ...
    interventions += [
        ...
        cv.test_prob(symp_prob=s_prob_august21, asymp_prob=0.004, symp_quar_prob=0.0, start_day=tti_day_august21, end_day=tti_day_sep21-1, test_delay=t_delay),
        cv.test_prob(symp_prob=s_prob_sep21, asymp_prob=0.008, symp_quar_prob=0.0, start_day=tti_day_sep21, test_delay=t_delay),
        ...
        cv.contact_tracing(trace_probs={'h': 1, 's': 0.8, 'w': 0.8, 'c': 0.1},
                           trace_time={'h': 0, 's': 1, 'w': 1, 'c': 2},
                           start_day='2021-07-12', end_day='2021-07-30',
                           quar_period=10),
        cv.contact_tracing(trace_probs={'h': 1, 's': 0.8, 'w': 0.8, 'c': 0.05},
                           trace_time={'h': 0, 's': 1, 'w': 1, 'c': 2},
                           start_day='2021-07-30', end_day='2022-07-19',
                           quar_period=10),
        ...
        cv.dynamic_pars({'iso_factor': {'days': tti_day_july21, 'vals': iso_vals7}}),
        cv.dynamic_pars({'iso_factor': {'days': tti_day_august21, 'vals': iso_vals8}}),
        cv.dynamic_pars({'iso_factor': {'days': tti_day_sep21, 'vals': iso_vals9}}),
...
if __name__ == '__main__':
    ...
    if whattorun=='quickfit':
        s0 = make_sim(seed=1, beta=0.0078, end_day='2021-11-30', verbose=0.1, vx_scenario=1)
        sims = []
        for seed in range(1):
            sim = s0.copy()
            sim['rand_seed'] = seed
        ...
        if save_sim:
                    #msim.reduce()
                    msim.reduce(quantiles=[0.25, 0.75])
                    msim.save(f'{resfolder}/uk_sim_calibration_16Oct.obj',keep_people=True)
        if do_plot:
            msim.plot(to_plot=to_plot, do_save=True, do_show=False, fig_path='England_calibration16oct.png',
                      legend_args={'loc': 'upper left'}, axis_args={'hspace': 0.4}, interval=75, n_cols=2)
            msim.plot('variants', do_save=True, do_show=False, fig_path='uk_strain_calibartion16oct.png')
    # Run scenarios
        ...
        for scenname in scenarios:
            for vx_scenario in vx_scenarios:
                ...
                print('---------------\n')
                sc.blank()
                s0 = make_sim(seed=1, beta=0.0078, end_day='2021-10-31', calibration=False, scenario=scenarios[0], vx_scenario=vx_scenario, verbose=0.1)
