...
#        rel_death_prob=1.5,
    ...
    beta_past  = sc.odict({'2020-02-14': [1.00, 1.00, 0.90, 0.90, ],
                           ...
                           '2020-10-26': [1.25, 0.00, 0.50, 0.70, ],
                           '2020-11-05': [1.25, sbv, 0.20, 0.30, ],
                           '2020-12-03': [1.25, sbv,  0.50, 0.70, ],
    ...
    if not calibration:
        ...
        beta_scens = sc.odict({'2021-01-04': [1.25, beta_s_jan4,  0.20, 0.30],
                               '2021-01-11': [1.25, beta_s_jan11, 0.20, 0.30],
                               '2021-01-18': [1.25, beta_s_jan18, 0.20, 0.30],
    ...
    interventions += [
        ...
        cv.test_prob(symp_prob=s_prob_sep, asymp_prob=0.02, symp_quar_prob=0.0, start_day=tti_day_sep, end_day=tti_day_oct-1, test_delay=t_delay),
        cv.test_prob(symp_prob=s_prob_oct, asymp_prob=0.02, symp_quar_prob=0.0, start_day=tti_day_oct, end_day=tti_day_nov-1, test_delay=t_delay),
        cv.test_prob(symp_prob=s_prob_nov, asymp_prob=0.02, symp_quar_prob=0.0, start_day=tti_day_nov, end_day=tti_day_dec-1, test_delay=t_delay),
        cv.test_prob(symp_prob=s_prob_dec, asymp_prob=0.02, symp_quar_prob=0.0, start_day=tti_day_dec, end_day=tti_day_jan-1, test_delay=t_delay),
        cv.test_prob(symp_prob=future_symp_test, asymp_prob=0.02, symp_quar_prob=0.0, start_day=tti_day_jan, test_delay=t_delay),
...
if __name__ == '__main__':
    ...
    # Run calibration with best-fitting seeds and parameters
        ...
        for bn, beta in enumerate(betas):
            for dn, delta_beta in enumerate(delta_betas):
                goodseeds = [i for i in range(n_runs) if fitsummary[bn][i] < 200] #231.5=100, 150=10
                sc.blank()
                print('---------------\n')
                print(f'Beta: {beta}, delta_beta: {delta_beta}, goodseeds: {len(goodseeds)}')
                print('---------------\n')
                if len(goodseeds) > 0:
                    s0 = make_sim(seed=1, beta=beta, delta_beta=delta_beta, end_day=data_end, verbose=0.1)
    ...
    # Run scenarios with best-fitting seeds and parameters
        ...
        for scenname in scenarios:
            ...
            for bn, beta in enumerate(betas):
                goodseeds = [i for i in range(n_runs) if fitsummary[bn][i] < 231.5]
                if len(goodseeds) > 0:
