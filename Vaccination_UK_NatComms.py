...
# Filepaths
data_path = 'UK_Covid_cases_march21.xlsx'
...
def make_sim(seed, beta, calibration=True, scenario=None, delta_beta=1.6, future_symp_test=None, end_day=None, verbose=0):
    ...
    if not calibration:
        ...
        if scenario == 'fullSchools':
            ...
            beta_scens = sc.odict({'2021-03-08': [1.25, sbv, 0.30, 0.40],
    ...
    voc_prop   = 0.52/(1+np.exp(-0.075*(voc_days-sim.day('2020-09-30')))) # Use a logistic growth function to approximate fig 2A of https://cmmid.github.io/topics/covid19/uk-novel-variant.html
    ...
    interventions += [
        ...
        cv.test_prob(symp_prob=future_symp_test, asymp_prob=0.0063, symp_quar_prob=0.0, start_day=tti_day_jan, end_day=tti_day_change-1, test_delay=t_delay),
        cv.test_prob(symp_prob=0.114, asymp_prob=0.0063, symp_quar_prob=0.0, start_day=tti_day_change, test_delay=t_delay),
        cv.contact_tracing(trace_probs={'h': 1, 's': 0.5, 'w': 0.5, 'c': 0.05},
                           trace_time={'h': 0, 's': 1, 'w': 1, 'c': 2},
                           start_day='2020-06-01', end_day='2021-02-28',
                           quar_period=10),
        #cv.contact_tracing(trace_probs={'h': 1, 's': 0.5, 'w': 0.5, 'c': 0.05},
        #                   trace_time={'h': 0, 's': 1, 'w': 1, 'c': 2},
    ...
    interventions += [utils.two_dose_daily_delayed(300e3, start_day='2020-12-20', dose_delay=21, delay=7*7,
...
if __name__ == '__main__':
    ...
    if whattorun=='quickfit':
        s0 = make_sim(seed=1, beta=0.0079, end_day='2021-04-10', verbose=0.1)
        sims = []
    ...
    # Run scenarios
        ...
        for scenname in scenarios:
            ...
            print('---------------\n')
            sc.blank()
            s0 = make_sim(seed=1, beta=0.0079, end_day='2021-06-30', calibration=False, scenario=scenname, verbose=0.1)
